...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    version="$(curl -fsSL https://api.github.com/repos/vinhnd1998/code-server/releases | awk 'match($0,/.*"html_url": "(.*\/releases\/tag\/.*)".*/)' | head -n 1 | awk -F '"' '{print $4}')"
  else
...
main() {
  ...
  # Fork's release tag (v4.177.0) does not match its asset filenames (4.117.0),
  # so VERSION (asset filename) and RELEASE_TAG (URL path) must be set separately.
  VERSION=${VERSION:-4.117.0}
  RELEASE_TAG=${RELEASE_TAG:-v4.177.0}
...
install_deb() {
  ...
  fetch "https://github.com/vinhnd1998/code-server/releases/download/$RELEASE_TAG/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://github.com/vinhnd1998/code-server/releases/download/$RELEASE_TAG/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://github.com/vinhnd1998/code-server/releases/download/$RELEASE_TAG/code-server-$VERSION-$OS-$ARCH.tar.gz" \
